...
public final class SystemTextInserter: TextInserting {
  ...
  public convenience init(
    ...
    pasteDelay: Duration = .milliseconds(30),
    ...
    clipboardRestoreDelay: Duration = .milliseconds(40)
  ) {
    self.init(
  ...
  public func insert(
    ...
    return try await pasteAndVerify(
      Self.separatedFromPrecedingText(text, before: before),
      target: target,
      before: before
    )
  }
  ...
  static func separatedFromPrecedingText(
    ...
    guard let value = before?.value, let range = before?.selectedRange, range.location > 0,
      let first = text.unicodeScalars.first,
      CharacterSet.alphanumerics.contains(first)
        || "\"“‘([".unicodeScalars.contains(first)
    else { return text }
    let nsValue = value as NSString
    guard range.location <= nsValue.length else { return text }
    let previousRange = nsValue.rangeOfComposedCharacterSequence(at: range.location - 1)
    guard let previous = nsValue.substring(with: previousRange).unicodeScalars.last,
  ...
  private func pasteAndVerify(
    ...
    pasteboard.clearContents()
    // Transient markers (nspasteboard.org) keep clipboard managers and history apps
    // from recording every dictation; current-host-only prevents Universal Clipboard sync.
...
private struct PasteboardSnapshot {
  ...
  /// Password managers mark secrets as concealed and clear them after a timeout keyed
  /// to the pasteboard change count. Writing them back would defeat that auto-clear.
  var isConcealed: Bool {
    items.contains { $0.values[Self.concealedType.rawValue] != nil }
  }
  ...
  func restore(to pasteboard: NSPasteboard) -> Bool {
    pasteboard.clearContents()
    guard !items.isEmpty, !isConcealed else { return true }
